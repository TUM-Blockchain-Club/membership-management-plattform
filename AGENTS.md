...
- Coffee Chats live under `/coffee-chats`, use `cc_rounds`, `cc_signups`, `cc_pairs`, and the private `coffee-chat-selfies` bucket. Apply `supabase/coffee_chats.sql` before deployment. Keep pairing commits atomic through `commit_coffee_chat_pairing`, validate selfie bytes server-side, and serve selfies only through short-lived signed URLs.
