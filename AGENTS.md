...
- Run `pnpm test` for the complete local test suite. Production Coffee Chats E2E is manual-only through the protected `production-e2e` GitHub environment; never place its credentials in local or committed files.
...
- Coffee Chats live under `/coffee-chats` inside the shared `(dashboard)` route group, use `cc_rounds`, `cc_signups`, `cc_pairs`, and the private `coffee-chat-selfies` bucket. Keep the dashboard shell persistent and derive one current-round action instead of adding a standalone feature shell. Apply `supabase/coffee_chats.sql` before deployment. Keep pairing commits atomic through `commit_coffee_chat_pairing`, validate selfie bytes server-side, require explicit meeting/selfie mutation intents, and serve selfies only through short-lived signed URLs.
- Coffee Chats production E2E may update and restore one dedicated test member's interests. Keep round creation, pairing, closing, and deletion out of automated production tests.
...
- Solana membership NFTs use Metaplex Core and the lifecycle documented in `docs/architecture/solana-membership-nfts.md`. Keep signer code server-only, source portraits private, public metadata versioned, and every chain mutation represented in `nft_chain_operations`.
