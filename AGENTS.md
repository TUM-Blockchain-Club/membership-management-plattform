...
- Native Gmail signature updates live under `/email-signature`, use the existing Supabase Google OAuth client with the `gmail.settings.basic` scope, and call Gmail only from `lib/email-signature/gmail.ts`. Keep provider tokens ephemeral and never fall back to a Gmail alias that does not match the signed-in platform email. See `docs/maintenance/email-signature-integration.md`.
