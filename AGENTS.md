...
- Link redirect analytics are sourced from `link_redirect_definitions` and `link_redirect_clicks`. Keep the `/link-analytics` page board-only server-side and do not send raw click rows to the client. Apply `supabase/link_redirect_assets.sql` before deploying changes that select link display labels or uploaded link images.
