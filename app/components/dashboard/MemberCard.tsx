...
export function MemberCard({ member, getPictureUrl, isHonorary = false, isAlumni = false, isAdvisor = false, canEdit = false, isOwnProfile = false, onEdit }: {
  ...
  return (
    <Card className={`backdrop-blur-md py-0 hover:border-white/30 transition-all duration-200 relative overflow-hidden ${
      isBoardMember
      ...
      {isBoardMember && (
        <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-10">
          <Badge variant="outline" className="bg-yellow-500/20 border-yellow-500/40 text-yellow-400">
      ...
      {isHonorary && (
        <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-10">
          <Badge variant="outline" className="bg-amber-500/30 border-amber-400/50 text-amber-200 backdrop-blur-sm">
      ...
      {isAlumni && (
        <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-10">
          <Badge variant="outline" className="bg-emerald-500/30 border-emerald-400/50 text-emerald-200 backdrop-blur-sm">
      ...
      {isAdvisor && (
        <div className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 z-10">
          <Badge variant="outline" className="bg-indigo-500/30 border-indigo-400/50 text-indigo-200 backdrop-blur-sm">
      ...
      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-2 sm:gap-3 md:gap-4 relative z-0">
        <div className="flex-shrink-0">
          <Avatar className={`size-14 sm:size-16 md:size-20 border-2 ${
        ...
        <div className="flex-1 min-w-0 text-center sm:text-left w-full sm:w-auto">
          ...
          <div className="mt-1.5 sm:mt-2 flex flex-wrap gap-1 sm:gap-2 justify-center sm:justify-start">
            {statusLabel && (
              <Badge variant="outline" className={`text-[10px] sm:text-xs ${statusLabel === 'Active' ? 'bg-green-500/20 border-green-500/40 text-green-400' : isHonorary ? 'bg-amber-500/30 border-amber-400/50 text-amber-300' : isAlumni ? 'bg-emerald-500/30 border-emerald-400/50 text-emerald-300' : isAdvisor ? 'bg-indigo-500/30 border-indigo-400/50 text-indigo-300' : 'bg-gray-500/20 border-gray-500/40 text-gray-400'}`}>
                {statusLabel}
              </Badge>
            )}
            {departmentLabel && (
              <Badge variant="outline" className={`text-[10px] sm:text-xs truncate max-w-full ${isHonorary ? 'bg-amber-500/20 border-amber-500/40 text-amber-300' : isAlumni ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-300' : isAdvisor ? 'bg-indigo-500/20 border-indigo-500/40 text-indigo-300' : 'bg-purple-500/20 border-purple-500/40 text-purple-400'}`}>
                {departmentLabel}
              </Badge>
            )}
          </div>
      ...
      <Separator className="mt-3 sm:mt-4 bg-white/10" />
      <div className="mt-3 sm:mt-4 relative z-0">
        <div className="flex flex-col sm:flex-row items-center sm:items-center justify-between gap-2">
          ...
          {canEdit && onEdit && (
            <Button
              ...
              size="sm"
              onClick={onEdit}
              className={`w-full sm:w-auto sm:ml-3 text-[10px] sm:text-xs ${isOwnProfile ? 'bg-blue-500/20 hover:bg-blue-500/30 border-blue-500/40 text-blue-300 hover:text-blue-200' : 'bg-purple-500/20 hover:bg-purple-500/30 border-purple-500/40 text-purple-300 hover:text-purple-200'}`}
            >
