...
## Private application preview

Selecting a portrait in the NFT application form renders a draft via authenticated `POST /api/nft-requests/draft-preview`. It uses the same `renderMembershipDraft` / `buildNftImage` pipeline as minting. Display name and Member Flex come from the form; Department, Batch and membership status are resolved server-side. Changing the input cancels the previous request and releases its browser object URL. The preview is explicitly a draft, not proof of approval or ownership.

The endpoint accepts raster portraits up to 3 MB and 25 megapixels, validates decoded content, and returns a private, non-cacheable PNG. Portraits are processed in memory only: no storage writes, request creation, public metadata or blockchain operations occur. The normal versioned publication-consent requirement remains unchanged. Portrait rotation/cropping is shared with minting for consistent output.
