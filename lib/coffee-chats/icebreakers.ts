/**
 * Ice-breaker question bank for TBC Coffee Chats.
 *
 * Questions are grouped by category.  getQuestionsForPair() selects three
 * questions tailored to the interests the two (or three) people share, with
 * universal questions as a reliable fallback.
 */
...
/** Maps interest keyword → question category. Case-insensitive substring match. */
...
function interestToCategory(interest: string): Category | null {
  ...
  for (const [keyword, cat] of INTEREST_TO_CATEGORY) {
...
/**
 * Returns exactly three ice-breaker questions for a matched pair (or trio).
 *
 * Tries to surface questions relevant to shared interests, then fills up with
 * TBC-specific and universal questions.
 */
export function getQuestionsForPair(
  person1Interests: string[],
  person2Interests: string[],
): [string, string, string] {
  const allInterests = [...person1Interests, ...person2Interests]
  const categories = new Set<Category>()
  for (const interest of allInterests) {
    const cat = interestToCategory(interest)
  ...
  // First: shared interest categories
  ...
  // Fill with TBC question if not already picked
  if (chosen.length < 3 && !usedCategories.has('tbc')) {
    chosen.push(pickRandom(QUESTIONS.tbc))
    usedCategories.add('tbc')
  }

  // Fill remaining with universal
  while (chosen.length < 3) {
    chosen.push(pickRandom(QUESTIONS.universal))
  }
