...
export const parseSignatureInput = (value: unknown): SignatureInput => {
  ...
  const linkedinUrl = parseLinkedInUrl(requireString(record, 'linkedinUrl', 'LinkedIn URL'))
  const mobileNumber = parseMobileNumber(requireString(record, 'mobileNumber', 'Mobile number'))
